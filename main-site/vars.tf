...
variable "apex_aliases" {
  description = "Map of apex hostnames to serve site on, to zone IDs"
  type        = map(string)
  default     = {}
}

variable "subdomains" {
  description = "List of subdomains to serve site on. E.g. with and without www"
  type        = list(any)
}
...
variable "cf_default_ttl" {
  description = "CloudFront default TTL for caching"
  type        = string
  default     = "0"
}
